...
describe('ES6 Analyzer', () => {
  ...
  it('returns an object with an empty array as value if no dependency found', () => {
    let dependencies = analyzer.generateDependencyObject(path.join(samplesPath,'es6-no-import.js'));
    assert.deepEqual(dependencies, ['es6-no-import']);
  });

  it('only check one file if the deps are node modules', () => {
    let dependencies = analyzer.generateDependencyObject(path.join(samplesPath,'es6-single-import.js'));
    assert.deepEqual(dependencies,['es6-single-import',['path']]);
  });

  it('support "as from" syntax', () => {
    let dependencies = analyzer.generateDependencyObject(path.join(samplesPath, 'es6-single-import-as-syntax.js'));
    assert.deepEqual(dependencies,['es6-single-import-as-syntax',['es6-no-import']]);
  });

  it('gets multiple dependencies in one file', () => {
    let dependencies = analyzer.generateDependencyObject(path.join(samplesPath, 'es6-multi-import-same-dir.js'));
    assert.deepEqual(dependencies,['es6-multi-import-same-dir',['es6-no-import','path']]);
  });

  it('gets one level of nested dependencies', () => {
    let dependencies = analyzer.generateDependencyObject(path.join(samplesPath, 'es6-nested-import-same-dir.js'));
  ...
  it('gets multiple level of nested dependencies', () => {
    let dependencies = analyzer.generateDependencyObject(path.join(samplesPath, 'es6-multiple-nested-same-dir.js'));
  ...
  describe('bugs', () => {
    it('outputs the right name independently of being written with single or double quotes', () => {
      let dependencies = analyzer.generateDependencyObject(path.join(samplesPath, 'es6-single-import-double-quote.js'));
