...
describe('Command line module', () => {
  ...
  it('calls the init function if the entry point of the app is passed', () => {
    let commandLineArguments = ['node', __dirname + '/dTrack.js', '-i', path.join(__dirname, '..', 'samples', 'es6-no-import.js')];
  ...
  it('supports the dot format', () => {
    let commandLineArguments = ['node', __dirname + '/dTrack.js', '-i',  path.join(__dirname, '..', 'samples', 'es6-no-import.js'), '-f', 'dot'];
    assert.deepEqual('digraph "es6-no-import" { "es6-no-import" }', dTrack.init(commandLineArguments));
  });
